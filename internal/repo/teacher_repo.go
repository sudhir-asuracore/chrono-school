...
func (r *TeacherRepo) Create(ctx context.Context, t *models.Teacher) error {
	...
	for _, qual := range t.Qualifications {
		_, err = tx.ExecContext(ctx, "INSERT INTO teacher_qualifications (teacher_id, subject_id, level_id) VALUES (?, ?, ?)", t.ID, qual.SubjectID, qual.LevelID)
		if err != nil {
			return err
		}
	}
...
func (r *TeacherRepo) List(ctx context.Context, orgID uuid.UUID) ([]models.Teacher, error) {
	...
	query := `SELECT * FROM teachers WHERE organization_id = ? ORDER BY id`
...
func (r *TeacherRepo) Get(ctx context.Context, id uuid.UUID) (*models.Teacher, error) {
	...
	err = r.db.SelectContext(ctx, &quals, "SELECT subject_id, level_id FROM teacher_qualifications WHERE teacher_id = ?", t.ID)
	if err == nil {
...
func (r *TeacherRepo) DeleteAll(ctx context.Context, orgID uuid.UUID) error {
	query := `DELETE FROM teachers WHERE organization_id = ?`
...
func (r *TeacherRepo) Update(ctx context.Context, t *models.Teacher) error {
	...
	_, err = tx.ExecContext(ctx, "DELETE FROM teacher_qualifications WHERE teacher_id = ?", t.ID)
	if err != nil {
		return err
	}
...
func (r *TeacherRepo) Delete(ctx context.Context, id uuid.UUID, orgID uuid.UUID) error {
	query := `DELETE FROM teachers WHERE id = ? AND organization_id = ?`
...
func (r *TeacherRepo) MarkAsStaleBySubject(ctx context.Context, orgID uuid.UUID, subjectID uuid.UUID) error {
	query := `UPDATE teachers SET is_stale = true 
	          WHERE organization_id = ? AND id IN (SELECT teacher_id FROM teacher_qualifications WHERE subject_id = ?)`
