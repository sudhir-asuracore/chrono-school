...
func (r *ClassRepo) Create(ctx context.Context, c *models.Class) error {
	...
	for _, item := range c.Curriculum {
		_, err = tx.ExecContext(ctx, "INSERT INTO curriculum_items (class_id, subject_id, periods_per_week, binding_id) VALUES (?, ?, ?, ?)", c.ID, item.SubjectID, item.PeriodsPerWeek, item.BindingID)
		if err != nil {
			return err
		}
	}
...
func (r *ClassRepo) List(ctx context.Context, orgID uuid.UUID) ([]models.Class, error) {
	...
	query := `SELECT * FROM classes WHERE organization_id = ? ORDER BY id`
...
func (r *ClassRepo) DeleteAll(ctx context.Context, orgID uuid.UUID) error {
	query := `DELETE FROM classes WHERE organization_id = ?`
...
func (r *ClassRepo) Get(ctx context.Context, id uuid.UUID) (*models.Class, error) {
	...
	var items []models.CurriculumItem
	err = r.db.SelectContext(ctx, &items, "SELECT * FROM curriculum_items WHERE class_id = ?", c.ID)
	if err == nil {
		c.Curriculum = items
	}
...
func (r *ClassRepo) Update(ctx context.Context, c *models.Class) error {
	...
	_, err = tx.ExecContext(ctx, "DELETE FROM curriculum_items WHERE class_id = ?", c.ID)
	if err != nil {
		return err
	}
...
func (r *ClassRepo) Delete(ctx context.Context, id uuid.UUID, orgID uuid.UUID) error {
	query := `DELETE FROM classes WHERE id = ? AND organization_id = ?`
